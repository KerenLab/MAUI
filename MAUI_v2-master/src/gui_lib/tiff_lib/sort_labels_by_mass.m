function [labels, runinfo] = sort_labels_by_mass(labels, path)
    ...
    %concatenate labels of antibodies with same metal - nofar
    G = findgroups(panelMass);
    panelLabel = splitapply(@(x1){strjoin(sort(x1) ,', ')},panelLabel,G);
